% Examples:
% T = powerwall2_old().read()
% powerwall2_old().download('2025-01-06')
% T = powerwall2_old().getData({'2025-01-01' -1});
% T = powerwall2_old().read([],{'2019-04-06' '2019-04-08'});

classdef powerwall2_old
    ...
    methods
        ...
        function obj = powerwall2_old(varargin)
            % Class constructor

            % Apply ini settings
            ini = fullfile(fileparts(mfilename('fullpath')), 'powerwall2.ini');
            if isfile(ini)
                txt = fileread(ini);
        ...
        function obj = authenticate(obj, site_ind)
            ...
            assert(~isempty(obj.refreshToken), 'Need a refreashToken to authenticate with Tesla API')
            ...
            obj.siteInfo = data.response(site_ind);
            obj.siteId = num2str(obj.siteInfo.energy_site_id);

            % Write info to file (optional)
            if ~isfolder(obj.dataFold)
                mkdir(obj.dataFold);
            end
            writelines(jsonencode(obj.siteInfo, 'PrettyPrint', true), fullfile(obj.dataFold, obj.siteId + "_info.json"))
        end
