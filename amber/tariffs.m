...
% Gets provider fees, optionally can add spot price (inc GST).
...
% Price data
fields = {'date' 'tariff'     'supply'      'buy_tod'     'buy_fees'                 'sell_tod' 'sell_fees' 'export_limit' 'rrp_buy' 'rrp_sell'};
data = {
    ...
    "2024-07-01" "Amber RTOU inc"  199.372 [ 1  6 10 15] [14.53234 28.12064  9.99484 28.12064] [0      ] [0      ] inf 1.3437281 1.221572 % testing GST inc
    "2025-07-01" "Amber RTOU inc"  191.29  [ 0  6 10 16] [16.15035 27.62115 10.42705 27.62115] [0 10 16] [0 1.1 0] inf 1.3064304 1.187665 

    "2024-07-01" "Amber RTOU inc 2"  181.24  [0] [0] [0] [0] inf 1.1 1.1
    "2025-07-01" "Amber RTOU inc 2"  191.29  [0] [0] [0] [0] inf 1.1 1.1
...
if nargin >= 3
    % Load AEMO wholesale rrp price if not provided
    if ~isempty(rrp) && (ischar(rrp) || isstring(rrp))
        switch(rrp)
            ...
            case 'amber'
                ...
                sell = nan(n, 1);
                valid = ind > 0;
                buy(valid) = T2.buy_price(ind(valid));
                sell(valid) = -T2.sell_price(ind(valid));
                'HACKS'
                return
        end
    end

    buy(tariff_ind>0)  = buy(tariff_ind>0)  + rrp(tariff_ind>0) .* T.rrp_buy(tariff_ind(tariff_ind>0));
    sell(tariff_ind>0) = sell(tariff_ind>0) - rrp(tariff_ind>0) .* T.rrp_sell(tariff_ind(tariff_ind>0));
end
