...
uibutton         (ht,         'Position', [W- 80    50    70    30], 'Text', 'Plot',               'ButtonPushedFcn', @(~,~)plotRows(hData),         'Tooltip', 'Plot data');
    ...
    function editData(h,e)
        ...
        elseif name == "Units"
            ...
            switch old + ">" + new
                case 'kw>kwh', T.(var) = T.(var) .* rez; T = renamevars(T, var, strrep(var, '_kw', '_kwh'));
                case 'kwh>kw', T.(var) = T.(var) ./ rez; T = renamevars(T, var, strrep(var, '_kwh', '_kw'));
                case '$>c',    T.(var) = T.(var) .* 100; T = renamevars(T, var, strrep(var, '_$', '_c'));
                case 'c>$',    T.(var) = T.(var) ./ 100; T = renamevars(T, var, strrep(var, '_c', '_$'));
            end
            if new == "custom"
                h.Data.Units{row} = [];
    ...
    function plotRows(h)
        ...
        for k = 1:n
            ...
            col = [T.Properties.CustomProperties.C1{rows(k)}; T.Properties.CustomProperties.C2{rows(k)}];
    ...
    function updateData(T)
        ...
        for k = width(T):-1:1
            ...
            switch T.Properties.VariableTypes(k)
                case {'double' 'single' 'int'}
                    info(k,:) = {min(t) max(t) mean(t,1,'omitmissing') range(t) sum(~isfinite(t))/height(T)*100};
                case {'datetime' 'duration'}
                    info(k,:) = {string(min(t)) string(max(t)+rez) "" round(days(range(t)+rez),1)+"d" sum(~isfinite(t))/height(T)*100};
            end
        end
        ...
        info = cell2table(info, 'VariableNames', {'Property' 'Label' 'Units' 'Type' 'Min' 'Max' 'Mean' 'Range' 'Fill %' 'C1' 'C2'});
        ...
        hData.ColumnWidth = {'auto' 'auto' 70 70 82 82 60 60 50 30 30};
        hData.Data = info;
        ...
        try
            ...
            if isempty(T)
                ...
                if ~isempty(duplicates)
                    fprintf(2, ' Overwriting duplicate columns: %s\n', strjoin(duplicates, ','))
...
if isempty(T.Properties.VariableDescriptions)
    t = strrep(strrep(T.Properties.VariableNames, "_" + T.Properties.VariableUnits, ''), "_" , ' ');
